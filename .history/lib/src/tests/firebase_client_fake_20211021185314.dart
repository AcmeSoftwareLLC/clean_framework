...
class FirebaseClientFake implements FirebaseClient {
  ...
  final _content;
  FirebaseClientFake([_content = const {'content': ''}]);
  ...
  Stream<Map<String, dynamic>> watch(
    ...
    if (_content['content']!.isEmpty)
      _content['content'] = _content['content']! + ' from watch id';
    Future.delayed(
  ...
  Stream<Map<String, dynamic>> watchAll({required String path}) {
    if (_content['content']!.isEmpty)
      _content['content'] = _content['content']! + ' from watch all';
    Future.delayed(
        Duration(milliseconds: 100), () => _controller.sink.add(_content));
    return _controller.stream;
  }
  ...
  void createQuery(String path, SnapshotQuery<Map<String, dynamic>> query) =>
      _content['content'] = 'with query ';

  @override
  clearQuery() => _content['content'] = '';
