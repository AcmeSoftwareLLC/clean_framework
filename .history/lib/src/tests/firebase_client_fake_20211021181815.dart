...
class FirebaseClientFake implements FirebaseClient {
  ...
  SnapshotQuery<Map<String, dynamic>>? _query;
  FirebaseClientFake();
  ...
  Stream<Map<String, dynamic>> watch(
    ...
    if (_content['content']!.isEmpty) _content['content'] = 'from watch id';
    Future.delayed(
  ...
  Stream<Map<String, dynamic>> watchAll({required String path}) {
    if (_content['content']!.isEmpty) _content['content'] = 'from watch all';
    Future.delayed(
        Duration(milliseconds: 100), () => _controller.sink.add(_content));
    return _controller.stream;
  }
