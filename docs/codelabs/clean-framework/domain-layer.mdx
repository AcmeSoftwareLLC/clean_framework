...
So, now we have a Use Case set up, and it's sending and receiving data from the Presenter properly. That's great, but what if we have two features that need to share data or communicate in some way? Luckily, we have a built-in way to do this: creating a bridge between two use cases.
