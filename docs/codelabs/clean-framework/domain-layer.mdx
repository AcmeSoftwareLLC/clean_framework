...
Awesome! Our Pokemon app is getting closer to actually having some logic. In the next section, we'll plug the gateway into the domain layer to prepare for interacting with external services (e.g. an a REST api).